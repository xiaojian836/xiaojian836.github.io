(function() {
		...
		while (current.length > 0) {
			...
			current.forEach(function(e) {
				...
				if (e.typ.named) {
					...
					if (e.indirect) {
						mset = mset.concat($ptrType(e.typ)
							.methods);
					}
				}
				switch (e.typ.kind) {
	...
	var $structType = function(pkgPath, fields) {
		...
		if (typ === undefined) {
			var string = "struct { " + $mapArray(fields, function(f) {
					return f.name + " " + f.typ.string + (f.tag !== "" ? (" \"" + f.tag.replace(/\\/g, "\\\\")
						.replace(/"/g, "\\\"") + "\"") : "");
				})
				.join("; ") + " }";
			if (fields.length === 0) {
	...
	$packages["runtime"] = (function() {
		...
		$ptrType(errorString)
			.prototype.RuntimeError = function() {
				return new errorString(this.$get())
					.RuntimeError();
			};
						...
						case 5:
							...
							rain2[0].Clicked = (function(rain1, rain2) {
								return function() {
									...
									$global.document.getElementById('CanvasBG')
										.style.background = $externalize((x$4 = (_r$6 = index % 2, _r$6 === _r$6 ? _r$6 : $throwRuntimeError("integer divide by zero")), ((x$4 < 0 || x$4 >= backgrounds.$length) ? ($throwRuntimeError("index out of range"), undefined) : backgrounds.$array[backgrounds.$offset + x$4])), $String);
								};
							})(rain1, rain2);
							$s = -1;
							return;
